...
public class BuildEventStreamerTest {
  ...
  public void testReferPastEvent() {
    ...
    BuildEvent startEvent = new GenericBuildEvent(
        ...
        ImmutableSet.of(ProgressEvent.INITIAL_PROGRESS_UPDATE),
        "initial event");
    BuildEvent earlyEvent = new GenericBuildEvent(
        new BuildEventId("unexpected"),
        ImmutableSet.of(),
        "unexpected event");
    BuildEvent lateReference = new GenericBuildEvent(
        new BuildEventId("late reference"),
        ImmutableSet.of(earlyEvent.getEventId()),
        "event referencing a previous event");
    ...
    for (BuildEvent event : eventsSeen) {
      if (event.getEventId().equals(earlyEvent.getEventId())) {
        // The early event should be passed through identically
        assertEquals(earlyEvent, event);
        earlyEventCount++;
      }
    }
