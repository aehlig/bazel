...
class ExperimentalStateTracker {
  ...
  private Deque<String> runningActions;
  private boolean ok;

  ExperimentalStateTracker() {
    this.runningActions = new ArrayDeque<String>();
    this.ok = true;
  }
  ...
  void buildComplete(BuildCompleteEvent event) {
    if (event.getResult().getSuccess()) {
      status = "INFO";
      additionalMessage = "Build completed succesfully, " + actionsCompleted + " total actions";
    } else {
      ok = false;
      status = "FAILED";
      additionalMessage = "Build did NOT complete succesfully";
    }
  }
