...
public class BuildEventStreamer implements EventHandler {
    ...
    synchronized(this) {
      if (announcedEvents == null) {
        ...
        if (!event.getChildrenEvents().contains(ProgressEvent.INITIAL_PROGRESS_UPDATE)) {
          ...
          announcedEvents.remove(linkEvent.getEventId());
        }
      } else {
  ...
  private void clearPendingEvents() {
    if (announcedEvents != null) {
      ...
      ImmutableSet<BuildEventId> ids;
      synchronized(this) {
        ids = ImmutableSet.copyOf(announcedEvents);
      }
      for (BuildEventId id : ids) {
        post(GenericBuildEvent.buildAborted(id));
      }
    }
  }
